...
def searchChords(userPrompt: str) -> str:
    try:
        search = app.search(query=userPrompt + " Ultimate Guitar", limit=1)
...
if __name__ == "__main__":
    userPrompt = str(input("Enter the song title and artist: "))
    song_chords = getChords(userPrompt)
    user_request = str(input("Enter your enhancement request (e.g., 'make it easier to play'): "))
    enhanced_content = getAIEnhancement(song_chords.lyrics_with_chords, user_request)
    print("Enhanced Lyrics with Chords:\n", enhanced_content)
